...
| `kyc.utils.ts`                            | `KycUtils` namespace with `canKycStatusModifyKycData()` — returns `false` when KYC is APPROVED, `true` otherwise. Centralizes the "can modify" check for immutable KYC data enforcement. |
| `__tests__/kyc.utils.spec.ts`             | Unit tests for `KycUtils` — tests all VentairyKycStatus enum values.                                                                                                                     |
