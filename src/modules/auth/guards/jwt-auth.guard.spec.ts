	...
	function createMockJwtService(shouldSucceed: boolean): JwtService {
		return {
			verifyAccessToken: shouldSucceed
				? vi.fn().mockResolvedValue({ sub: "u-1", sid: "s-1", user_type: UserType.BUSINESS, wallet_address: "0xabc", chain_id: 8453, kyc_status: VentairyKycStatus.APPROVED, iat: 1000, exp: 2000 })
...
describe("JwtAuthGuard", () => {
	describe("canActivate", () => {
		...
		it("should set request.user with id, sessionId, userType, walletAddress, chainId, and kycStatus from payload", async () => {
			const guard = new JwtAuthGuard(createMockReflector(false), createMockJwtService(true));
